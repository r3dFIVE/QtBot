...
PresenceUpdateTest::test_deserialization_minimal() {
    PresenceUpdate presenceUpdate;
    presenceUpdate.fromQString(PLD_PRESENCE_UPDATE_MINIMAL);

    User user;
    user.fromQJsonObject(presenceUpdate.getUser());
    ...
    ClientStatus clientStatus;
    clientStatus.fromQJsonObject(presenceUpdate.getClientStatus());
    QVERIFY(clientStatus.getDesktop() == TEST_STRING1);
    QVERIFY(presenceUpdate.getPremiumSince() == "");
    QVERIFY(presenceUpdate.getNick() == "");
}

void
PresenceUpdateTest::test_deserialization_full() {
    PresenceUpdate presenceUpdate;
    presenceUpdate.fromQString(PLD_PRESENCE_UPDATE_FULL);
    ...
    QVERIFY(presenceUpdate.getRoles()[1].toString() == TEST_STRING2);
    Activity game;
    game.fromQJsonObject(presenceUpdate.getGame());
